...
from plugins_func.functions.get_giddy_weather import maintain_weather_cache
...
async def main():
    ...
    # 启动 Simple http 服务器
    ota_server = SimpleHttpServer(config, device_controller=ws_server)
    ota_task = asyncio.create_task(ota_server.start())
    weather_warm_task = asyncio.create_task(
        maintain_weather_cache(
            config.get("plugins", {}).get("get_giddy_weather", {})
        )
    )
    ...
    try:
        ...
        stdin_task.cancel()
        ws_task.cancel()
        weather_warm_task.cancel()
        if ota_task:
            ota_task.cancel()

        # 等待任务终止（必须加超时）
        await asyncio.wait(
            [stdin_task, ws_task, ota_task, weather_warm_task]
            if ota_task
            else [stdin_task, ws_task, weather_warm_task],
            timeout=3.0,
